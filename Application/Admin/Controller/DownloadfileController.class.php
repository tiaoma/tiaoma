...
class DownloadfileController extends BaseController {
    ...
    public function post(){
        ...
        if(IS_POST){
          ...
          if(!$id){
           ...
           $data['addtime'] = time();
           $mod->add($data);
           $this->success('添加成功',U('Downloadfile/index'));
           die;
          }
          
          $mod->save($data);
          $this->success('修改成功',U('Downloadfile/index'));
          die;
        }
        if($id){
         $info = $mod->where("id='{$id}' and del=0")->find();
             if($info){
                $this->assign("info",$info);
                 ...
                 $this->error('参数错误',U('Downloadfile/index'));
            }
        }
    ...
    public function delAction(){      
        ...
        $this->success('删除成功',U('Downloadfile/index'));
    }  
