...
<nav class="breadcrumb"><i class="iconfont">&#xf012b;</i> 首页 <span class="c-gray en">&gt;</span> Banner图片 <span class="c-gray en">&gt;</span> 图片列表 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="icon-refresh"></i></a></nav>
  ...
  <table class="table table-striped table-bordered bootstrap-datatable datatable">
                    ...
                    <tbody> 
                        <?php if( $list): if(is_array($list)): foreach($list as $key=>$info): ?><tr>
                                ...
                                <td>
                                       <img src="/sptmcx/Public/Data/others<?php echo ($info[href]); ?>" alt="" width="100"> 
                                    </td>
                                <td><?php echo ($info[title]); ?></td>
                                <td><?php echo (date('Y-m-d',$info[createtime])); ?></td>
                                <td>
                                    <?php if( $info[isvideo]==1): ?>是 <?php else: ?>  否<?php endif; ?>     
                                </td> 
                                <td>
                                    <?php echo ($placeName[$info[place]]); ?>
                                </td>  
                                <td><?php echo ($niStatusName[$info[status]]); ?></td>                        
                                <td  class="f-14">
                                    <a  href="<?php echo U('Banner/edit',array('id'=>$info[id],'rand'=>rand(1,10000)));?>"  data-name="<?php echo ($info[title]); ?>" data-id="<?php echo ($info[id]); ?>">
                                        <i class="icon-edit"></i>          
                                    </a>
                                    <a  onclick="class_del(this)" class="ml-5" data-url="<?php echo U('Banner/del',array('id'=>$info[id],'rand'=>rand(1,10000)));?>" href="###">
