...
<nav class="breadcrumb"><i class="iconfont">&#xf012b;</i> 首页 <span class="c-gray en">&gt;</span>  友情链接 <span class="c-gray en">&gt;</span><?php if($bdata): ?>编辑<?php else: ?>添加<?php endif; ?><a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="icon-refresh"></i></a></nav>
<div class="pd-20">
  <form class="Huiform" action="" method="post" enctype="multipart/form-data">
  <table class="table">
      <input type="hidden" value="<?php echo ($bdata[id]); ?>" name="id" />
      <tr>
        <th width="100">名称:</th>
        <td><input type="text" value="<?php echo ($bdata[title]); ?>" class="form-control" name="title"></td>
      </tr>
      <tr>
        <th width="100">链接:</th>
        <td><input type="text" value="<?php echo ($bdata[link]); ?>" class="form-control" name="link"></td>
      </tr>
       <tr >
        <th width="100">排序:</th>
        <td><input   value="<?php echo ($bdata['sorder']); ?>" maxlength="64" class="input-text" placeholder="填写的数字越大越靠前"  style="width:250px" name="sorder"></td>
    </tr>
      <tr>
        <th width="100">状态:</th>
        <td>
          <label class="radio-inline" >
            <input type="radio" id="inline-radio2" name="status" value="1" <?php if($bdata['status'] == 1): ?>checked<?php endif; ?>> 显示
          </label>
          <label class="radio-inline">
            <input type="radio" id="inline-radio2" name="status" value="0" <?php if($bdata['status'] == 0): ?>checked<?php endif; ?>> 隐藏
          </label>
        </td>
      </tr> 
      <tr>
        <th width="100">类型:</th>
        <td>
          <select class="select" id="friend_type" name="friend_type" onchange="SetSubID(this);">            
            <?php if(is_array($friendLinks)): foreach($friendLinks as $fid=>$vo): ?><option value="<?php echo ($fid); ?>" <?php if($bdata['friend_type'] == $fid): ?>selected=""<?php endif; ?>><?php echo ($vo); ?></option><?php endforeach; endif; ?>      
          </select>
        </td>
      </tr> 
  </table>
      ...
      <a href="<?php echo U('Friendlink/index');?>"><button type="button" class="btn btn-success" id="" name="" > 返回列表</button></a>
    </div>
    </form>
